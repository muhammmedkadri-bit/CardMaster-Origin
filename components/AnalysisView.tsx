...
const AnalysisView: React.FC<AnalysisViewProps> = ({ cards, transactions, isDarkMode, onBack, onEditTransaction, onDeleteTransaction, categories, lastUpdate }) => {
  ...
  return (
    <div className="relative min-h-screen">
      {!isContentReady && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-50 dark:bg-[#0b0f1a] transition-opacity duration-300">
          <div className={`w-12 h-12 border-4 border-t-transparent rounded-full animate-spin ${isDarkMode ? 'border-blue-500' : 'border-blue-600'}`}></div>
          <p className="mt-4 text-slate-400 font-bold tracking-widest text-xs uppercase animate-pulse">Analiz Yükleniyor...</p>
        </div>
      )}
      <div className={`pt-0 pb-0 transition-opacity duration-500 ${isContentReady ? 'opacity-100' : 'opacity-0'}`}>
        {/* Header Section */}
        ...
        <div id="analysis-transactions" className="grid grid-cols-1 gap-8 mb-0 scroll-mt-[1px]">
