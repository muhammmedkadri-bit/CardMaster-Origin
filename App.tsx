...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 pb-40 sm:pb-32 overflow-x-hidden ${isDarkMode ? 'bg-[#0b0f1a] text-slate-100' : 'bg-[#f8fafc] text-slate-900'}`}>
      ...
      {!isInitialLoading && (
        <div key="bottom-nav-morph-v6" className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[2100] flex flex-col items-center w-full max-w-[360px] sm:max-w-none sm:w-auto pointer-events-none animate-morph-reveal">
          <div className="pointer-events-auto flex flex-col items-center w-full sm:px-0">
            ...
            <div className={`flex items-center gap-0.5 sm:gap-3 sm:p-2.5 rounded-[24px] sm:rounded-[40px] border-2 shadow-2xl ${isDarkMode ? 'bg-[#0b0f1a]/95 border-slate-700 shadow-black' : 'bg-white/95 border-slate-300 shadow-blue-900/10'}`}>
              {/* Left buttons */}
              <div className="flex items-center gap-0.5 p-1.5 sm:px-2 sm:gap-2">
                <button onClick={() => handleViewChange('dashboard')} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-lg sm:rounded-xl transition-all duration-300 active:scale-95 ${view === 'dashboard' ? (isDarkMode ? 'bg-slate-700 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-black/20 translate-y-[1px]' : 'bg-white text-blue-600 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)] border border-slate-200/50 translate-y-[1px]') : (isDarkMode ? 'text-slate-400 hover:text-slate-300 border border-transparent' : 'text-slate-500 hover:text-slate-600 border border-transparent')}`} aria-label="Dashboard"><LayoutDashboard size={16} className="sm:w-5 sm:h-5" /></button>
                <button onClick={() => handleViewChange('cards')} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-lg sm:rounded-xl transition-all duration-300 active:scale-95 ${view === 'cards' ? (isDarkMode ? 'bg-slate-700 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-black/20 translate-y-[1px]' : 'bg-white text-blue-600 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)] border border-slate-200/50 translate-y-[1px]') : (isDarkMode ? 'text-slate-400 hover:text-slate-300 border border-transparent' : 'text-slate-500 hover:text-slate-600 border border-transparent')}`} aria-label="Cards"><CardIcon size={16} className="sm:w-5 sm:h-5" /></button>
                <button onClick={() => handleViewChange('analysis')} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-lg sm:rounded-xl transition-all duration-300 active:scale-95 ${view === 'analysis' ? (isDarkMode ? 'bg-slate-700 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-black/20 translate-y-[1px]' : 'bg-white text-blue-600 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)] border border-slate-200/50 translate-y-[1px]') : (isDarkMode ? 'text-slate-400 hover:text-slate-300 border border-transparent' : 'text-slate-500 hover:text-slate-600 border border-transparent')}`} aria-label="Analysis"><BarChart3 size={16} className="sm:w-5 sm:h-5" /></button>
              </div>

              {/* FAB button */}
              <button
                onClick={() => setIsFabOpen(!isFabOpen)}
                className={`shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-white shadow-2xl transition-all duration-500 active:scale-95 ${isFabOpen ? (isDarkMode ? 'bg-gradient-to-tr from-slate-600 to-slate-400 scale-90' : 'bg-gradient-to-tr from-slate-500 to-slate-300 scale-90') : 'bg-gradient-to-tr from-blue-700 to-blue-500 shadow-blue-500/40 hover:scale-105'}`}
                aria-label="New Transaction"
              >
                <Plus size={24} className={`sm:w-9 sm:h-9 transition-transform duration-400 ${isFabOpen ? 'rotate-[135deg]' : 'rotate-0'}`} strokeWidth={3} />
              </button>

              {/* Right buttons */}
              <div className="flex items-center gap-0.5 p-1.5 sm:px-2 sm:gap-2">
                <button onClick={() => handleViewChange('settings')} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-lg sm:rounded-xl transition-all duration-300 active:scale-95 ${view === 'settings' ? (isDarkMode ? 'bg-slate-700 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-black/20 translate-y-[1px]' : 'bg-white text-blue-600 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)] border border-slate-200/50 translate-y-[1px]') : (isDarkMode ? 'text-slate-400 hover:text-slate-300 border border-transparent' : 'text-slate-500 hover:text-slate-600 border border-transparent')}`} aria-label="Settings"><SettingsIcon size={16} className="sm:w-5 sm:h-5" /></button>
                <button onClick={() => setIsDarkMode(!isDarkMode)} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-lg sm:rounded-xl transition-all duration-300 ${isDarkMode ? 'text-slate-400 hover:text-yellow-400' : 'text-slate-500 hover:text-blue-600'}`} aria-label="Toggle Theme">
                ...
                <button
                  ...
                  className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-lg sm:rounded-xl transition-all duration-300 ${isDarkMode ? 'text-slate-400 hover:text-rose-400' : 'text-slate-500 hover:text-rose-600'}`}
                  title={user ? `${user.email} - Çıkış Yap` : 'Giriş Yap'}
                  aria-label={user ? "Logout" : "Login"}
                >
                  {user ? <LogOut size={16} className="sm:w-5 sm:h-5" /> : <LoginIcon size={16} className="sm:w-5 sm:h-5" />}
                </button>
              </div>
            </div>
          </div>
