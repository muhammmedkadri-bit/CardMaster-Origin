...
const App: React.FC = () => {
  ...
  const [categories, setCategories] = useState<Category[]>(() => {
    ...
    const defaultCategories: Category[] = [
      ...
      { id: '7', name: 'Faiz & Ek Ücretler', color: '#F43F5E' },
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 pb-[142px] sm:pb-[162px] overflow-x-hidden ${isDarkMode ? 'bg-[#0b0f1a] text-slate-100' : 'bg-[#f8fafc] text-slate-900'}`}>
      ...
      <div
        className={`fixed inset-0 z-[99999] bg-[#f8fafc] dark:bg-[#0b0f1a] flex items-center justify-center transition-opacity duration-500 pointer-events-none ${isPageTransitioning ? 'opacity-100 pointer-events-auto' : 'opacity-0'}`}
      >
      ...
      {!isInitialLoading && (
        <div key="bottom-nav-morph-v6" className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[2100] flex flex-col items-center w-full max-w-[400px] sm:max-w-none sm:w-auto pointer-events-none animate-morph-reveal ">
          <div className="pointer-events-auto flex flex-col items-center w-full sm:px-0">
            ...
            <div className={`flex items-center gap-1 py-3 px-2 sm:gap-3 sm:px-3 rounded-full border-[1.5px] shadow-2xl ${isDarkMode ? 'bg-[#0b0f1a]/95 border-slate-800 shadow-black' : 'bg-white/95 border-slate-200 shadow-blue-900/10'}`}>
              {/* Left buttons */}
              <div className="flex items-center gap-1 p-1 sm:px-2 sm:gap-2">
                ...
                <button onClick={() => handleViewChange('cards')} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-full sm:rounded-xl transition-all duration-300 active:scale-95 ${view === 'cards' ? (isDarkMode ? 'bg-slate-700 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-black/20 translate-y-[1px]' : 'bg-white text-blue-600 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)] border border-slate-200/50 translate-y-[1px]') : (isDarkMode ? 'text-slate-400 hover:text-slate-300 border border-transparent' : 'text-slate-500 hover:text-slate-600 border border-transparent')}`} aria-label="Cards"><CardIcon size={18} className="sm:w-5 sm:h-5" /></button>
                <button onClick={() => handleViewChange('analysis')} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-full sm:rounded-xl transition-all duration-300 active:scale-95 ${view === 'analysis' ? (isDarkMode ? 'bg-slate-700 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-black/20 translate-y-[1px]' : 'bg-white text-blue-600 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)] border border-slate-200/50 translate-y-[1px]') : (isDarkMode ? 'text-slate-400 hover:text-slate-300 border border-transparent' : 'text-slate-500 hover:text-slate-600 border border-transparent')}`} aria-label="Analysis"><BarChart3 size={18} className="sm:w-5 sm:h-5" /></button>
              </div>

              {/* FAB button */}
              <button
                onClick={() => setIsFabOpen(!isFabOpen)}
                className={`shrink-0 w-[52px] h-[52px] sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-white shadow-2xl transition-all duration-500 active:scale-95 ${isFabOpen ? (isDarkMode ? 'bg-gradient-to-tr from-slate-600 to-slate-400 scale-90' : 'bg-gradient-to-tr from-slate-500 to-slate-300 scale-90') : 'bg-gradient-to-tr from-blue-700 to-blue-500 shadow-blue-500/40 hover:scale-105'}`}
              ...
              {/* Right buttons */}
              <div className="flex items-center gap-1 p-1 sm:px-2 sm:gap-2">
                <button onClick={() => handleViewChange('settings')} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-full sm:rounded-xl transition-all duration-300 active:scale-95 ${view === 'settings' ? (isDarkMode ? 'bg-slate-700 text-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] border border-black/20 translate-y-[1px]' : 'bg-white text-blue-600 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)] border border-slate-200/50 translate-y-[1px]') : (isDarkMode ? 'text-slate-400 hover:text-slate-300 border border-transparent' : 'text-slate-500 hover:text-slate-600 border border-transparent')}`} aria-label="Settings"><SettingsIcon size={18} className="sm:w-5 sm:h-5" /></button>
                <button onClick={() => setIsDarkMode(!isDarkMode)} className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-full sm:rounded-xl transition-all duration-300 ${isDarkMode ? 'text-slate-400 hover:text-yellow-400' : 'text-slate-500 hover:text-blue-600'}`} aria-label="Toggle Theme">
                ...
                <button
                  ...
                  className={`flex items-center justify-center w-[42px] h-[42px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-full sm:rounded-xl transition-all duration-300 ${isDarkMode ? 'text-slate-400 hover:text-rose-400' : 'text-slate-500 hover:text-rose-600'}`}
