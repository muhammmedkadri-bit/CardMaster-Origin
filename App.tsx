...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 pb-40 sm:pb-32 overflow-x-hidden ${isDarkMode ? 'bg-[#0b0f1a] text-slate-100' : 'bg-[#f8fafc] text-slate-900'}`}>
      ...
      {!isInitialLoading && (
        <div key="bottom-nav-morph-v6" className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[2100] flex flex-col items-center w-full max-w-[400px] sm:max-w-none sm:w-auto pointer-events-none animate-morph-reveal">
            ...
            <div className={`flex items-center gap-3 sm:gap-4 p-2 sm:p-2.5 rounded-full border shadow-2xl ${isDarkMode ? 'bg-[#0b0f1a]/95 border-slate-700 shadow-black' : 'bg-white/95 border-slate-300 shadow-blue-900/10'}`}>
              ...
              <div className="flex items-center gap-2 p-1 sm:px-2 sm:gap-2">
                ...
                <button
                  ...
                  className={`flex items-center justify-center w-[52px] h-[52px] sm:px-4 sm:py-3 sm:w-auto sm:h-auto rounded-full sm:rounded-xl transition-all duration-300 ${isDarkMode ? 'text-slate-400 hover:text-rose-400' : 'text-slate-500 hover:text-rose-600'}`}
